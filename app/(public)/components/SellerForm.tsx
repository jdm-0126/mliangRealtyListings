...
function validate(v: Values): Errors {
  ...
  else if (!validateContactNumber(v.contactNumber.trim()))
    e.contactNumber = 'Enter a valid Philippine mobile number.'
  if (!v.email.trim()) e.email = 'Email address is required.'
  else if (!/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(v.email.trim()))
    e.email = 'Enter a valid email address.'
  if (!v.propertyAddress.trim())
    e.propertyAddress = 'Property address or description is required.'
  return e
}
  ...
  if (status === 'success') {
    return (
      ...
      >
        ...
        >
          Thank you! We'll be in touch to discuss your property.
        </p>
      </div>
    )
  }
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="space-y-5">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label htmlFor="s-fullName" style={labelStyle}>
            Full Name <span style={{ color: 'var(--est-purple)' }}>*</span>
          </label>
          <input
            id="s-fullName"
            name="fullName"
        ...
        <div>
          <label htmlFor="s-contactNumber" style={labelStyle}>
            Contact Number <span style={{ color: 'var(--est-purple)' }}>*</span>
          </label>
          <input
            id="s-contactNumber"
            name="contactNumber"
      ...
      {/* Email */}
      <div>
        <label htmlFor="s-email" style={labelStyle}>
          Email Address <span style={{ color: 'var(--est-purple)' }}>*</span>
        </label>
        <input
          id="s-email"
          name="email"
      ...
      {/* Property address */}
      <div>
        <label htmlFor="s-propertyAddress" style={labelStyle}>
          Property Address / Description{' '}
          <span style={{ color: 'var(--est-purple)' }}>*</span>
        </label>
        <input
          id="s-propertyAddress"
          name="propertyAddress"
          type="text"
          value={values.propertyAddress}
          onChange={handleChange}
          maxLength={300}
          placeholder="e.g. Lot 12, Brgy. Dolores, San Fernando, Pampanga"
          style={errors.propertyAddress ? fieldError : fieldBase}
          disabled={status === 'loading'}
        />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
        <div>
          <label htmlFor="s-propertyType" style={labelStyle}>
            Property Type
          </label>
          <select
            id="s-propertyType"
        ...
        <div>
          <label htmlFor="s-listingIntent" style={labelStyle}>
            I want to
          </label>
          <select
            id="s-listingIntent"
            name="listingIntent"
            value={values.listingIntent}
            onChange={handleChange}
            style={selectStyle}
            disabled={status === 'loading'}
          >
      ...
      {/* Asking price */}
      <div>
        <label htmlFor="s-askingPrice" style={labelStyle}>
          Asking Price{' '}
          <span style={{ color: 'var(--est-border)' }}>(optional)</span>
        </label>
        <input
          id="s-askingPrice"
          name="askingPrice"
          ...
          onChange={handleChange}
          maxLength={50}
          placeholder="e.g. ₱2,500,000 or negotiable"
          style={fieldBase}
          disabled={status === 'loading'}
        />
      </div>
      ...
      {/* Additional details */}
      <div>
        <label htmlFor="s-details" style={labelStyle}>
          Additional Details{' '}
          <span style={{ color: 'var(--est-border)' }}>(optional)</span>
        </label>
        <textarea
          ...
          onChange={handleChange}
          maxLength={1000}
          placeholder="Lot area, floor area, number of bedrooms, special features, reason for selling…"
