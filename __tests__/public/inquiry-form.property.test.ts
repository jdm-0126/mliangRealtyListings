...
// Mock Supabase — mockInsert must be set up per test
  ...
  it('does not call insert when fullName is empty', async () => {
    await fc.assert(
      fc.asyncProperty(validContactNumber, validEmail, validMessage, async (cn, email, msg) => {
        ...
        const result = mockInsert.mock.calls.length === 0
        unmount()
        return result
      }),
      { numRuns: 20 }
...
describe('Property 14: Valid form submissions insert correct data', () => {
  ...
  it('calls insert exactly once with correct fields and valid ISO 8601 created_at', async () => {
    await fc.assert(
      fc.asyncProperty(
        ...
        async (values) => {
          ...
          await act(async () => submitForm(container))
          await waitFor(() => expect(mockInsert.mock.calls.length).toBeGreaterThan(0), { timeout: 3000 })
